...
def synthesis_fullfield( oim, nfp, xs, ys, write_fits = True ):
    """
    Reconstructs and synthesizes a full-field image from atom files, computes residuals 
    and other relevant products, and optionally writes the results to a FITS file.

    Parameters:
    -----------
    oim : numpy.ndarray
        The original input image as a 2D array. It is used to compute residuals by comparing 
        with the reconstructed image.
    
    nfp : str
        The file path prefix for accessing the segmented HDF5 files used in reconstruction, and 
        for saving the FITS output file if `write_fits` is True.
    
    xs : int
        The x-dimension size for the output images (e.g., reconstructed, residual, weights).
    
    ys : int
        The y-dimension size for the output images.

    write_fits : bool, optional, default=True
        If True, writes the synthesized results (original, reconstructed, residual, weights, 
        detection error images) to a FITS file at the specified path `nfp`.

    Returns:
    --------
    tuple : (rec, res, wei, dei)
        - rec : numpy.ndarray
            The reconstructed image generated by combining segments from HDF5 files.
        - res : numpy.ndarray
            The residual image, computed as the difference between `oim` and `rec`.
        - wei : numpy.ndarray
            The weight map indicating regions with detected objects.
        - dei : numpy.ndarray
            The detection error map.
    """
